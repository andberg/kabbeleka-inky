  ...
  var Path$1 = function () {
    ...
    createClass(Path, [{
      ...
      get: function get() {
        ...
        if (this.isRelative) return "." + compsStr;
        else return compsStr;
      },
  ...
  var RawList = function () {
    ...
    createClass(RawList, [{
      ...
      value: function MaxAsList() {
        if (this.Count > 0) return new RawList(this.maxItem);
        else return new RawList();
      }
    }, {
      key: 'MinAsList',
      value: function MinAsList() {
        if (this.Count > 0) return new RawList(this.minItem);
        else return new RawList();
      }
    }, {
      key: 'Equals',
      ...
      get: function get() {
        ...
        if (this.Count > 0) {
          ...
          else this._originNames.length = 0;
    ...
    createClass(ListValue, [{
      ...
      value: function Cast(newType) {
        if (newType == ValueType.Int) {
          ...
          else return new FloatValue(parseFloat(max.Value));
  ...
  var VariableReference = function (_InkObject) {
    ...
    createClass(VariableReference, [{
      ...
      set: function set(value) {
        ...
        else this.pathForCount = new Path$1(value);
      }
    }]);
    ...
    createClass(JsonSerialisation, null, [{
      ...
      value: function JTokenToRuntimeObject(token) {
        ...
        if (typeof token === 'string') {
          ...
          // Glue
          if (str == "<>") return new Glue(GlueType.Bidirectional);
          else if (str == "G<") return new Glue(GlueType.Left);
          else if (str == "G>") return new Glue(GlueType.Right);
      ...
      value: function RuntimeObjectToJToken(obj) {
        ...
        if (divert instanceof Divert) {
          ...
          if (divert.isExternal) divTypeKey = "x()";
            ...
            if (divert.stackPushType == PushPopType.Function) divTypeKey = "f()";
            else if (divert.stackPushType == PushPopType.Tunnel) divTypeKey = "->t->";
          }

          var targetStr;
          if (divert.hasVariableTarget) targetStr = divert.variableDivertName;
          else targetStr = divert.targetPathString;
    ...
    createClass(StoryState, [{
      ...
      value: function TrimFromExistingGlue() {
        ...
        while (i < this._outputStream.length) {
          ...
          if (txt instanceof StringValue && !txt.isNonWhitespace) this._outputStream.splice(i, 1);
          else i++;
        }
      ...
      get: function get() {
        for (var i = this._outputStream.length - 1; i >= 0; i--) {
          ...
          //			var glue = c as Glue;
          var glue = c;
          if (glue instanceof Glue) return i;
          else if (c instanceof ControlCommand) // e.g. BeginString
            break;
        }
        return -1;
      ...
      get: function get() {
        for (var i = this._outputStream.length - 1; i >= 0; i--) {
          ...
          //			var glue = c as Glue;
          var glue = c;
          if (glue instanceof Glue && glue.isRight) return glue;
          else if (c instanceof ControlCommand) // e.g. BeginString
            break;
        }
        return null;
      ...
      set: function set(value) {
        if (value != null) this.currentContentObject = this.story.ContentAtPath(value);
        else this.currentContentObject = null;
      }
    }, {
    ...
    createClass(Story, [{
      ...
      value: function EvaluateFunction(functionName, args, returnTextOutput) {
        ...
        try {
          ...
          if (e.message.indexOf("not found") >= 0) throw "Function doesn't exist: '" + functionName + "'";
          else throw e;
        }
