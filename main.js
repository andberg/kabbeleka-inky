(function (storyContent) {
  ...
  function ProcessTag(tag, delay) {
    ...
    if (tag.indexOf('stopMusic') == 0) {
      var existingContent = storyContainer.querySelectorAll('audio');
    ...
    if (tag.indexOf('clear') == 0) {
      var existingContent = storyContainer.querySelectorAll(
        'p, a, img, video, audio',
      );
      if (delay <= 200.0) {
        for (var i = 0; i < existingContent.length; i++) {
  ...
  function continueStory() {
    ...
    while (story.canContinue) {
      ...
      if (
        story.currentTags[0] == 'load' &&
        localStorage.getItem('inkParagraphText') !== null
      ) {
